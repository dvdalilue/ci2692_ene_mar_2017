...
class Reproductor(QWidget):
	...
	def Actual(self):
		...
		if self.output.state() == QAudio.ActiveState or self.output.state() == QAudio.SuspendedState:
			return self.__playlist.proxima.anterior.elemento

		return self.__playlist.proxima.elemento
	
	def Ordenar_Artista(self):
		self.__playlist = self.__playlist.ordenar_artista()
	
	def Ordenar_Titulo(self):
		self.__playlist = self.__playlist.ordenar_titulo()
